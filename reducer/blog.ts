...
const blog = createSlice({
    ...
    reducers: {
        loading: (state, action) => {
            const reqAction = action as PayloadAction<IBlog>;
            return { ...state, ...reqAction.payload };
        },
        addUploadFiles: (state, action) => {
            const pre = state.uploadFileInfo;
            return { ...state, uploadFileInfo: [...pre, ...action.payload] };
        },
    ...
    extraReducers: (builder) => {
        builder.addCase(getCategoriMenu.pending, (state, action) => {
            return {
                ...state,
                loading: true,
            };
